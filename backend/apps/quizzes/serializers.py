...
class QuizSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Quiz
        fields = ['id', 'title', 'subject', 'duration', 'questions', 'question_ids', 'created_at']

    def validate(self, data):
        if data.get('duration', 1) <= 0:
            raise serializers.ValidationError({'duration': 'Duration must be greater than zero.'})
        if not data.get('questions'):
            raise serializers.ValidationError({'questions': 'Quiz must contain at least one question.'})
        return data


class AdminQuizSerializer(QuizSerializer):
...
class AttemptSerializer(serializers.ModelSerializer):
    class Meta:
        model = QuizAttempt
        fields = '__all__'
        read_only_fields = ['student', 'score', 'percentage', 'completed_at']
